...
map.plot <- function(
	...
	for (i in seq_along(tracks)) {
		...
		lat <- lat[mask]
		lines <- SpatialLines(
			list(Lines(Line(cbind(lon,lat)), ID='lines')),
			crs.lonlat
		)
	...
	text(coordinates(spTransform(pts, crs)), geo.labels,
		adj=c(1, -0.5),
		cex=0.7,
		font=3
	)
	...
	for (i in seq_along(regions)) {
		...
		lon <- region[[2]][,1]
		lat <- region[[2]][,2]
		spt <- spTransform(SpatialPolygons(list(Polygons(list(Polygon(cbind(
				c(
			...
			))), 'regions')), proj4string=crs.lonlat), crs)
		plot(spt,
			border='#4b5effa0',
			lwd=2,
			add=TRUE
...
map <- function(tracks, output, ...) {
	...
	par(mar=c(0,0,0,0))
	land <- readOGR(LAND, layer='ne_50m_land')
	ds <- list()
	for (track in tracks) {
